...
package totem.resource
{
	...
	import totem.resource.providers.FallbackResourceProvider;
	import totem.resource.providers.IResourceProvider;
	...
	public class ResourceManager
	{
		...
		public function load( filename : String, resourceType : Class, onLoaded : Function = null, onFailed : Function = null, forceReload : Boolean = false ) : Resource
		{
			...
			if ( !resource )
			{
				...
				var fileExtension : String = TotemUtil.getFileExtension ( filename ).toLocaleLowerCase ();
			...
			else if ( resource.isLoaded )
			{
				if ( onLoaded != null )
				{					
					//TotemUtil.callLater ( onLoaded, [ resource ] );
				}
			}
			else
			{
				...
				if ( onLoaded != null )
					resource.addEventListener ( ResourceEvent.LOADED_EVENT, function( event : Event ) : void
					{
						onLoaded ( resource );
					} );
		...
		private function fail( resource : Resource, onFailed : Function, message : String ) : void
		{
			...
			//if ( onFailed != null )
				//TotemUtil.callLater ( onFailed, [ resource ] );
		}
