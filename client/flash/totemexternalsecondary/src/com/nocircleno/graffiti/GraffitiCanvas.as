...
package com.nocircleno.graffiti
{
	...
	*/
	...
	public class GraffitiCanvas extends Sprite
	{
		...
		*
		*/
		public function GraffitiCanvas( canvasWidth : uint = 100, canvasHeight : uint = 100, numberHistoryLevels : uint = 0, backgroundColor : uint = 0, bitmapData : BitmapData = null, overlay : DisplayObject = null, underlay : DisplayObject = null )
		{
			...
			if ( !bitmapData )
			{
				_bmp = new BitmapData( _canvasWidth, _canvasHeight, false, backgroundColor );
				canvas = new Bitmap( _bmp, "auto", false );
			}
			else
			{
				canvas = new Bitmap( bitmapData, "auto", false );
			}
		...
		public function loadCanvas( bitmapData : BitmapData, backgroundColor : uint = 0 ) : void
		{
		...
		*/
		public function nextHistory() : void
		{
			...
			if ( _history != null )
			{
				...
				if ( historyPosition != _history.length - 1 )
				{
					historyPosition++;
		...
		public function fill( point : Point, color : uint, useEntireCanvas : Boolean = false, useAdvancedFill : Boolean = true, smoothStrength : int = 8 ) : void
		{
			...
			if ( _bmp.rect.containsPoint( point ))
			{
				...
				if ( compareResult != 0 )
				{
					...
					dispatchEvent( new DrawEvent ( DrawEvent.UPDATE ) );
				}
		...
		*/
		public function clearCanvas() : void
		{
			...
			// clear canvas
			//0x00FFFFFF
			_bmp.fillRect( new Rectangle( 0, 0, _canvasWidth, _canvasHeight ), backgroundColor );
		...
		***************************************************************************/
		private function resizeCanvas() : void
		{
			...
			if ( _bmp != null )
			{
				...
				_bmp = new BitmapData( _canvasWidth, _canvasHeight, true, backgroundColor );
		...
		***************************************************************************/
		private function writeToHistory() : void
		{
			...
			if ( _historyPosition != historyLength - 1 )
			{
				...
				for ( i = ( historyLength - 1 ); i > historyPosition; --i )
				{
					_history[ i ].dispose();
					_history.splice( i, 1 );
				}
