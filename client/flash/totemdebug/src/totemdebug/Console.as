...
package totemdebug
{
	...
	public class Console extends Sprite implements ILogAppender
	{
		...
		//protected var _currentGroup : TotemGroup = null; //Smash._rootGroup;
		...
		public function initialize() : void
		{
			...
			_currentGroup = null; //Smash._rootGroup;
		...
		/*
		protected function _listSmashObjects( current : TotemObject, indent : int ) : int
		{
			if ( !current )
				return 0;
			...
			if ( current.getName() )
			{
				Logger.print( this, Console.generateIndent( indent ) + current.getName() + type );
			}
			else
			{
			...
			var parentSet : TotemSet = current as TotemSet;
			var parentGroup : TotemGroup = current as TotemGroup;
			var parentEntity : TotemEntity = current as TotemEntity;
			...
			else if ( parentGroup )
			{
				for ( i = 0; i < parentGroup.length; i++ )
					sum += _listSmashObjects( parentGroup.getTotemObjectAt( i ), indent + 1 );
			}
			else if ( parentEntity )
			{
				// Get all the components. Components don't count for the sum.
				/*var c : Vector.<Component> = parentEntity.getAllComponents();

				for ( i = 0; i < c.length; i++ )
				{
					var iec : Component = c[ i ] as Component;
		...
		public function listDirectory() : void
		{
			...
			{
				var potentialGroup : TotemGroup = _currentGroup.getTotemObjectAt( i ) as TotemGroup;

				if ( potentialGroup == null )
					continue;

				Logger.print( this, "#" + i + ". " + TypeUtility.getClass( _currentGroup ) + " name = " + _currentGroup.getName() );
			}
		}
		...
		public function changeDirectory( dir : String = null ) : void
		{
			...
			else
			{
				...
				for ( var i : int = 0; i < _currentGroup.length; i++ )
				{
					...
					if ( !potentialGroup.getName() || potentialGroup.getName().toLocaleLowerCase() != dir )
						continue;
				...
				{
					potentialGroup = _currentGroup.getTotemObjectAt( i ) as TotemGroup;
				...
				{
					_currentGroup = _currentGroup.getTotemObjectAt( dirAsNumber ) as TotemGroup;
		...
		*/
		...
		public function onTick() : void
		{
			...
			if ( bottomLineIndex == int.MAX_VALUE )
				startLine = TotemUtil.clamp( logCache.length - lineHeight, 0, int.MAX_VALUE );
			else
				startLine = TotemUtil.clamp( bottomLineIndex - lineHeight, 0, int.MAX_VALUE );

			endLine = TotemUtil.clamp( startLine + lineHeight, 0, logCache.length - 1 );
