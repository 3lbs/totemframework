...
*/
package totem.monitors.startupmanager.model
{
	import totem.monitors.startupmanager.events.StartupEvent;
	import totem.monitors.startupmanager.interfaces.IResourceList;
	import totem.monitors.startupmanager.interfaces.IRetryParameters;
	import totem.monitors.startupmanager.interfaces.IRetryPolicy;
	...
	*
	*/
	public class StartupMonitorProxy extends EventDispatcher
	{
		...
		*/
		 ...
		 */
		public var waitingForMoreResourcesNotificationName :String = StartupEvent.WAITING_FOR_MORE_RESOURCES;
		...
		*/
		public var prefixIfAutoSRPName :String = PREFIX_IF_AUTO_SRP_NAME;
		...
		*/
		...
		public function addResource( r :StartupResourceProxy ):void {
			if ( resourceList.isOpen() ) {
				...
				doSendNotification( StartupEvent.CALL_OUT_OF_SYNC_IGNORED );
		}
		/**
		...
		public function closeResourceList() :void {
			...
			if ( allResourcesLoaded() ) {
				finishThisTry( StartupEvent.LOADING_COMPLETE );
			}
		}
		/**
		...
		public function resourceLoaded( proxyName :String ):void {
			if ( ! loadingActive)
				doSendNotification( StartupEvent.CALL_OUT_OF_SYNC_IGNORED, proxyName );
			else
		...
		private function doResourceLoaded( proxyName :String ):void {
			...
			if ( r.isOkToSetLoaded() ) {
				...
				// Else are we finished, though all not loaded?
					...
					finishThisTry( StartupEvent.LOADING_FINISHED_INCOMPLETE );                
				}
		...
		public function sendProgressNotification() :void {
			doSendNotification( StartupEvent.LOADING_PROGRESS, resourceList.progressPercentage );
		}
		...
		private function doResourceFailed( proxyName :String, allowRetry :Boolean ):void {
			...
			if ( r.isOkToSetFailed() ) {
				...
				if ( allowRetry && r.isOkToRetry() ) {
					...
					doSendNotification( StartupEvent.RETRYING_LOAD_RESOURCE, r.appResourceProxyName() );
				}
				else {
		...
		internal function resourceHasBeenTimedOut( r :StartupResourceProxy ):void {
			// Notify the client app
			doSendNotification( StartupEvent.LOAD_RESOURCE_TIMED_OUT, r.appResourceProxyName() );
		...
		protected function removeResourceProxies() :void {
			for( var i:int = 0; i < this.resourceList.length; i++) {
				var r :StartupResourceProxy = this.resourceList.getItemAt(i) as StartupResourceProxy;
					//acade.removeProxy( r.getProxyName() );
					// Eddie need something above
		...
		protected function doSendNotification( event:String, body:Object=null ) :void {
			// sendNotification( notificationName, body, jobId );
			
			dispatchEvent( new StartupEvent( event ) );
		}
