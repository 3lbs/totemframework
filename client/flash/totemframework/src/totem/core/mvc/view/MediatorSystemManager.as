package totem.core.mvc.view
{
	...
	public class MediatorSystemManager extends TotemObject
	{
		 ...
		 */
		public function MediatorSystemManager( instance : TotemContext )
		{
			super("");
			_instance = instance;
			initialize();
		...
		override public  function initialize() : void
		{
			if ( _instance.getInjector() )
				_injector = _instance.getInjector().createChildInjector();
		...
		private function createMediator( view : Object, viewClass : Class, mediatorClass : Class ) : void
		{
			if ( _classes[ getClassName( view )] && !_mediators[ view ])
			{
				...
				if ( _injector )
				{
					...
					mediator = Mediator( _injector.createInstance( mediatorClass ));
					_injector.removeMapping( viewClass );*/
					
					getInjector().map( viewClass ).toValue( view );
					mediator = getInjector().getInstance( mediatorClass );
					getInjector().unmap( viewClass );
				}
				else
				{
