...
*/
package totem.patterns.mvc.controllers.macrobot.core
{
	...
	public class MacroBase extends AsyncCommand
	{
		...
		protected function prepareCommandFromDescriptor( descriptor : CommandDescriptor ) : Object
		{
			...
			if ( commandMap is IMacroCommandMap )
			{
				command = IMacroCommandMap( commandMap ).prepare( descriptor.commandClass, descriptor.payload, descriptor.payloadClass, descriptor.named );
			}
			...
			else
			{
				...
				if ( descriptor.payload )
				{
					...
					injector.map( payloadClass, descriptor.named ).toValue( descriptor.payload );

					injector.map( descriptor.commandClass ).asSingleton();
					command = injector.getInstance( descriptor.commandClass );
					injector.unmap( descriptor.commandClass );
...
{
	public function CommandDescriptor( commandClass : Class, payload : Object = null, payloadClass : Class = null, named : String = '' )
	{
