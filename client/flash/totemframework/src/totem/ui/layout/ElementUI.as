...
package totem.ui.layout
{
	...
	import totem.ui.layout.vo.PropertiesUI;
	...
	public class ElementUI extends EventDispatcher
	{
		...
		protected function refreshElement() : void
		{
			...
			if ( eventBeforeCalculation.isDefaultPrevented())
				return;
		...
		protected function setProperties( properties : PropertiesUI ) : void
		{
			if ( !isNaN( properties.x ))
				_object.x = properties.x;

			if ( !isNaN( properties.y ))
				_object.y = properties.y;

			if ( !isNaN( properties.width ))
				_object.width = properties.width;

			if ( !isNaN( properties.height ))
				_object.height = properties.height;
			setNegativeScale();
			setRatioOutMask();
		}
		...
		{
			if ( value == RATIO_IN || value == RATIO_OUT )
				_ratio = value;
			else
			{
				_ratio = null;
