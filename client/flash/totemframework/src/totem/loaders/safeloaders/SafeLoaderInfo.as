...
package totem.loaders.safeloaders
{
	 ...
	 */
	public class SafeLoaderInfo extends RemovableEventDispatcher
	{
		...
		override public function destroy() : void
		{
			...
			if ( _realLI )
			{
				_realLI.removeEventListener( Event.COMPLETE, handleLoaderInfoEvent );
				_realLI.removeEventListener( HTTPStatusEvent.HTTP_STATUS, handleLoaderInfoEvent );
				_realLI.removeEventListener( Event.INIT, handleLoaderInfoEvent );
				_realLI.removeEventListener( IOErrorEvent.IO_ERROR, handleLoaderInfoEvent );
				_realLI.removeEventListener( Event.OPEN, handleLoaderInfoEvent );
				_realLI.removeEventListener( ProgressEvent.PROGRESS, handleProgressEvent );
				_realLI.removeEventListener( Event.UNLOAD, handleLoaderInfoEvent );

				_realLI = null;
			}

			_loadDoneCallback = null;
		...
		private function handleAddedEvent( e : Event ) : void
		{
			...
			if ( c != null && p != null && ( c.parent == p || c == p ))
			{
				_numAdded++;
			}
			...
			if ( _numAdded > 1 )
			{
				if ( e.currentTarget.hasEventListener( Event.ADDED ) )
					e.currentTarget.removeEventListener( Event.ADDED, handleAddedEvent );
				
				if ( e.currentTarget.hasEventListener( FlexEvent.APPLICATION_COMPLETE ) )
					e.currentTarget.removeEventListener( FlexEvent.APPLICATION_COMPLETE, handleAddedEvent );
			...
			switch ( e.type )
			{
				...
				case Event.INIT:
					...
					try
					{
						...
						else if ( theName.indexOf( "_mx_managers_SystemManager" ) > -1 )
						{
							_rslPreloaderLoaded = true;
							_numAdded = 1;
							theContent.addEventListener( FlexEvent.APPLICATION_COMPLETE, handleAddedEvent );
						}
					}
					catch ( e : Error )
					{
