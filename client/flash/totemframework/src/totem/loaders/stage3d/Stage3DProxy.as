...
package totem.loaders.stage3d
{
	...
	public class Stage3DProxy extends EventDispatcher
	{
		...
		totem_internal var _context3D : Context3D;

		totem_internal var _stage3DIndex : int = -1;
		...
		private var getContext : GetContext3D;
		...
		public function Stage3DProxy( stage3DIndex : int, stage3D : Stage3D, stage3DManager : Stage3DManager, forceSoftware : Boolean = false )
		{
			...
			//getContext = new GetContext3D( _stage3D, handleContext )
		}
		...
		*/
		...
		{
			// force setting null
			if ( buffer && _activeVertexBuffers[ index ] == buffer )
				return;
		...
		{
			if ( texture != null && _activeTextures[ index ] == texture )
				return;
		...
		*/
		...
		private function handleContext( success : Boolean, message : String ) : void
		{
		...
		*/
		...
		private function requestContext( forceSoftware : Boolean = false ) : void
		{
			...
			var profile : String = ( MobileUtil.isHD()) ? Context3DProfile.BASELINE_EXTENDED : Context3DProfile.BASELINE_CONSTRAINED;

			var mode : String = forceSoftware ? Context3DRenderMode.SOFTWARE : Context3DRenderMode.AUTO;
